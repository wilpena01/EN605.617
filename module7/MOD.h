			 ...
			 UInt32 *Block, UInt32 *Thread)
{
	...
	if(g_input2 != 0)
		...
		Result[thread_idx] = InvalidNumber;
...
void Topmod_stream(UInt32 *gpu_arr1, UInt32 *gpu_arr2, UInt32 num_blocks, 
              UInt32 num_threads, RESULT *finalResult)
{
    //Preparation to do the modulo in the kernel using different streams
	...
	mod_arr<<<num_blocks, num_threads, 1, stream1>>>(gpu_arr1, gpu_arr2, gpu_Result, 
										 gpu_Block, gpu_Thread);
	mod_arr<<<num_blocks, num_threads, 1, stream2>>>(gpu_arr1, gpu_arr2, gpu_Result, 
										 gpu_Block, gpu_Thread);
	mod_arr<<<num_blocks, num_threads, 1, stream3>>>(gpu_arr1, gpu_arr2, gpu_Result, 
										 gpu_Block, gpu_Thread);
              ...
              UInt32 num_threads, RESULT *finalResult)
{
	...
	mod_arr_Reg<<<num_blocks, num_threads>>>(gpu_arr1, gpu_arr2, gpu_Result, 
										 gpu_Block, gpu_Thread);
	mod_arr_Reg<<<num_blocks, num_threads>>>(gpu_arr1, gpu_arr2, gpu_Result, 
										 gpu_Block, gpu_Thread);
	mod_arr_Reg<<<num_blocks, num_threads>>>(gpu_arr1, gpu_arr2, gpu_Result, 
